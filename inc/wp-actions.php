...
function lasntgadmin_show_quota_to_client()
{
	...
	foreach ($items as $item => $values) {
		$orders = QuotaUtil::get_orders_ids_by_product_id($values['data']->get_id());
		break;
	}


	if($orders){
		echo "<h3>Available quota remaining: $orders</h3>";
	}
}


function stock_filter($stock)
{
	$post_id = get_the_ID();
	$orders = QuotaUtil::get_orders_ids_by_product_id($post_id, ['wc-completed']);
	return $orders . " in stock";
}
add_filter("woocommerce_get_availability_text", "stock_filter", 10, 1);
...
{
	$orders = QuotaUtil::get_orders_ids_by_product_id($product_id);
	
	if ($orders != -1 && $product_quantity > $orders) {
		wc_add_notice(__('The max quantity you can enter is ' . $orders), 'error');
		return false;
	}
	return true;
}
