...
class ProductActionsFilters
{
	...
	public static function init(): void
	{
		...
		//media library.
		add_filter('ajax_query_attachments_args', [self::class, 'wpb_show_current_user_attachments']);
	...
	public static function wp_enqueue_media()
	{
		...
		error_log("UserGroups" . json_encode($user_groups));
...
<?php
	...
	public static function wpb_show_current_user_attachments($query)
	{
		...
		if (isset($_POST['query']) && isset($_POST['query']['lasntg_group'])) {
			$user_groups = GroupUtils::get_group_ids_by_user_id($user_id);
			if (is_admin() || in_array($group, $user_groups) !== false) {
				$group_id = $_POST['query']['lasntg_group'];
